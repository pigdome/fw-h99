...
class CreditTransectionSearch extends CreditTransection
{
        ...
        if ($this->from == 'frontend') {
            ...
            if (!empty($this->create_at)) {
                $query->andFilterWhere([
                    'between',
                    'create_at',
                    date("Y-m-d 00:00:00", strtotime($this->create_at)),
                    date("Y-m-d 23:59:59", strtotime($this->create_at))
                ]);
            } else {
                $query->andFilterWhere([
                    '<',
    ...
    public function checkCreditMasterBalance($action, $reason, $amount = 0)
    {
        ...
        if (
            in_array($action, [Constants::action_credit_top_up, Constants::action_credit_withdraw, Constants::action_credit_top_up_admin, Constants::action_credit_withdraw_admin, Constants::action_commission_withdraw_direct])
