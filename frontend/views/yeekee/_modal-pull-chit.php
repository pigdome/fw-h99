						...
						<br>
							<div class="table-responsive">
    							<table class="table table-bordered">
    								...
    								<tbody>
    									<?php foreach($resultChitList as $chitMap){
    										$chit = $chitMap['model'];
    										$chitJson = $chitMap['map_val'];
    									?>
    									<tr class="">
    										<th scope="row">#<?= $chit->getOrder()?></th>
    										<td>จับยี่กีรอบที่ <?= $chit->yeekee->round?> / <?= date('d/m/Y',strtotime($chit->yeekee->date_at))?></td>
    										<td><?= date('d/m/Y H:i',strtotime($chit->create_at))?></td>
