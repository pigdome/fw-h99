...
function two_option() {
    ...
    if (!showhide) {
        ...
        if ($.inArray("option_2_19", curr_option) >= 0) {
            remove_option("option_2_19");
        }
        if ($.inArray("option_2_roodnha", curr_option) >= 0) {
            remove_option("option_2_roodnha");
        }
        if ($.inArray("option_2_roodlung", curr_option) >= 0) {
            remove_option("option_2_roodlung");
        }
    } else {
...
$(document).keyup(async function (e) {
    if ($(".d-none").attr('data-id') === 'numpage_2') {
        if ($('#price').hasClass('pagemodal-wrapper open')){
            return true;
        }
        if ($('#printpoy').hasClass('pagemodal-wrapper open')){
            return true;
        }
        if ($('#sendpoy').hasClass('pagemodal-wrapper open')){
            return true;
        }
        if ($('#numpage_2').hasClass('d-none')){
            return true;
        }
        var textStr
        let chr;
        ...
        if (chr) {
            ...
            if (chr === "Backspace" && i_y > 0) {
                ...
                // $('.show-input-number').val(cur_num + 1);
                cur_num++;
            }
            if (i_y < 5) {
                i_y++;
            ...
            notify('เพิ่มเลขได้ครั้งละ 5 หลักเท่านั้น', "error", {autoHideDelay: false, position: "top center"});
        }
    }else {
...
function show_bet_num() {
    ...
    $.each(poy.poy_list, function (index, value) {
        ...
        if (bld['discount'][op] > 0) {
            total_price += discountTotal.toFixed(2) * 1;
         }else {
            total_price += price * 1;
        }
        bet_cnt++;
