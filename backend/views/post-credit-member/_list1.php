...
<?php //Pjax::begin(); ?>
    ...
    <div class="col-md-12" style="overflow-y:auto; background-color: #fff;">
        <?php Pjax::begin(['id' => 'post-credit-member-current']) ?>
        <?= GridView::widget([
            ...
            'columns' => [
                ...
                [
                    ...
                    'value' => function ($model) {
                        return isset($model->userHasBankUser->bank->title) ?
                            $model->userHasBankUser->bank->title :
                            $model->createBy->userHasBank->bank->title;
                ...
                [
                    'label' => 'โอนเข้าธนาคาร',
                    'value' => function ($model) {
                           return isset($model->userHasBank->bank->title) ?
                            $model->userHasBank->bank->title : '';
                    }
                ],
                [
                    'label' => 'แจ้งเมื่อ',
                    ...
                    'value' => function ($model) {
                        ...
                        if (Constants::status_waitting == $model->status) {
                            $result .= Html::a(Yii::t('app', ' {modelClass}', [
                                'modelClass' => 'ดำเนินการ',
                            ]), ['post-credit-member/updatestatus', 'id' => $model->id, 'type' => 'approve', 'active' => 'Current'],
                                ['class' => 'btn btn-xs btn-info',
                                    'data' => [
                                        'confirm' => Yii::t('app', 'Are you sure proceed ?'),
                        ...
                        if (Constants::status_waitting == $model->status) {
                            ...
                            $result .= Html::a(Yii::t('app', ' {modelClass}', [
                                'modelClass' => 'ยกเลิก',
                            ]), ['post-credit-member/cancel', 'id' => $model->id],
